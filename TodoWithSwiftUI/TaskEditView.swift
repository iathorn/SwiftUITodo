...
struct TaskEditView : View {
    ...
    var body: some View {
        ...
        return VStack(alignment: .leading, spacing: 0) {
            TextField(self.draftTitle.binding, placeholder: Text("Enter New Title..."), onEditingChanged: { _ in self.updateTask() }, onCommit: {})
                .background(
                    RoundedRectangle(cornerRadius: 5)
                        .fill(Color.clear)
                        .border(
                            Color(red: 0.7, green: 0.7, blue: 0.7),
                            width: 1 / UIScreen.main.scale, cornerRadius: 5)
                        .padding(inset)
                )
                .padding(EdgeInsets(
                    top: 15 - inset.top,
